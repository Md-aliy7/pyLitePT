...
try:
    ...
    if not found:
        # Try to find torch_scatter_cpu in parent directory components
        params = os.path.dirname(os.path.realpath(__file__)).split(os.sep)
        # Search up to 3 levels up
        for i in range(3):
            path = os.sep.join(params[:-i-1])
            if os.path.exists(os.path.join(path, 'torch_scatter_cpu.py')):
                ...
                try:
                    ...
                    print("Using local torch_scatter_cpu fallback")
                    found = True
                    break
                except ImportError:
                    pass
    ...
    if not found:
        ...
        try:
            import torch_scatter_cpu as torch_scatter
            print("Using torch_scatter_cpu from path")
        except ImportError:
            print("Warning: torch_scatter not found and fallback failed.")
            torch_scatter = None

from addict import Dict
from collections import OrderedDict
from models.utils.structure import Point
# Mock HookBase to remove legacy 'engines' dependency
class HookBase:
    """Mock for legacy engines.hooks.HookBase."""
    pass
