...
try:
    ...
    if not found:
        # Try to find spconv_cpu in parent directory components
        # litept.py is deeply nested, search up to 4 levels
...
class LitePT(PointModule):
    def __init__(
        self,
        in_channels: int = 4,
        order: Union[str, Tuple[str, ...]] = ("z", "z-trans", "hilbert", "hilbert-trans"),
        ...
        dec_conv: Tuple[bool, ...] = (False, False, False, False),
    ...
    def forward(self, data_dict):
        ...
        if self.enc_attn[0]:
            # Only shuffle during training to ensure consistent eval results
            do_shuffle = self.shuffle_orders and self.training
            point.serialization(order=self.order, shuffle_orders=do_shuffle)
        point.sparsify()
