...
class RandomRotate(object):
    ...
    def __call__(self, data_dict):
        ...
        if "gt_boxes" in data_dict.keys():
            ...
            # Use the same center as coord if available, otherwise calculate from boxes (or 0)
            if center is None:
                 if self.center is None:
                      # If no coord, use box center? or 0? 
                      # Usually we rotate around the scene center. 
                      # For now fallback to box center/min-max mean
                      b_min = data_dict["gt_boxes"][:, :3].min(axis=0)
                      b_max = data_dict["gt_boxes"][:, :3].max(axis=0)
                      center = (b_min + b_max) / 2
                 else:
                      center = self.center
            
            # Rotate Box Centers
            data_dict["gt_boxes"][:, :3] -= center
            data_dict["gt_boxes"][:, :3] = np.dot(data_dict["gt_boxes"][:, :3], np.transpose(rot_t))
            data_dict["gt_boxes"][:, :3] += center
            
            # Rotate Heading (assuming index 6 is heading, and Z-axis rotation)
            if self.axis == "z":
                data_dict["gt_boxes"][:, 6] += angle
                # Normalize angle to [-pi, pi] if needed, but OpenPCDet usually handles it.
                # Just in case:
                # data_dict["gt_boxes"][:, 6] = (data_dict["gt_boxes"][:, 6] + np.pi) % (2 * np.pi) - np.pi
        return data_dict
