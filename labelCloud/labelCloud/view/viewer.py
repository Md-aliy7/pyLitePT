...
class GLWidget(QtOpenGL.QGLWidget):
    ...
    def get_world_coords(
        ...
        # Custom gluUnProject implementation for Python 3.12 compatibility
        # OpenGL matrices are column-major, transpose to row-major for numpy
        try:
            modelview = np.array(self.modelview, dtype=np.float64).reshape(4, 4).T
            projection = np.array(self.projection, dtype=np.float64).reshape(4, 4).T
            ...
            in_vec = np.array([
                (float(x) - vp[0]) / vp[2] * 2.0 - 1.0,
                (float(real_y) - vp[1]) / vp[3] * 2.0 - 1.0,
                2.0 * float(z) - 1.0,
                1.0
            ], dtype=np.float64)
            ...
            mod_x, mod_y, mod_z = float(out_vec[0]), float(out_vec[1]), float(out_vec[2])
        except Exception:
            mod_x, mod_y, mod_z = 0.0, 0.0, 0.0
