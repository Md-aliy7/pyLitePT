...
def get_pick_ray(x: float, y: float, modelview, projection) -> Tuple[Point3D, Point3D]:
    ...
    # Custom gluUnProject for Python 3.12 compatibility
    p_front = _custom_unproject(x, real_y, 0, modelview, projection, viewport)
    p_back = _custom_unproject(x, real_y, 1, modelview, projection, viewport)
    return p_front, p_back


def _custom_unproject(win_x, win_y, win_z, modelview, projection, viewport):
    """Pure numpy implementation of gluUnProject for Python 3.12 compatibility.
    
    OpenGL stores matrices in column-major order. When glGetDoublev returns them,
    they need to be transposed to work correctly with numpy's row-major arrays.
    """
    try:
        # OpenGL matrices are column-major, transpose to row-major for numpy
        mv = np.array(modelview, dtype=np.float64).reshape(4, 4).T
        proj = np.array(projection, dtype=np.float64).reshape(4, 4).T
        vp = np.array([viewport[0], viewport[1], viewport[2], viewport[3]], dtype=np.float64)
        
        # Combined transform: clip = proj @ mv @ vertex
        # Inverse: vertex = inv(proj @ mv) @ clip
        m = proj @ mv
        m_inv = np.linalg.inv(m)
        
        # Map window coordinates to normalized device coordinates [-1, 1]
        in_vec = np.array([
            (float(win_x) - vp[0]) / vp[2] * 2.0 - 1.0,
            (float(win_y) - vp[1]) / vp[3] * 2.0 - 1.0,
            2.0 * float(win_z) - 1.0,
            1.0
        ], dtype=np.float64)
        
        # Transform back to object coordinates
        out_vec = m_inv @ in_vec
        
        if abs(out_vec[3]) < 1e-10:
            return (0.0, 0.0, 0.0)
        
        # Perspective divide
        out_vec = out_vec / out_vec[3]
        return (float(out_vec[0]), float(out_vec[1]), float(out_vec[2]))
    except Exception:
        return (0.0, 0.0, 0.0)
