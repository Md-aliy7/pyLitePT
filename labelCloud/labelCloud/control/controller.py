...
class Controller:
    ...
    # CONTROL METHODS
    def save(self) -> None:
        """Saves all bounding boxes and exports data in organized format.
        
        Export format (ply/npy_folder) and split (train/val/test) are set
        in the startup dialog and saved to the EXPORT config section.
        
        Output structure:
        segmentations/
        ├── train/
        │   ├── scene.ply  OR  scene/coord.npy, color.npy, segment.npy, gt_boxes.npy
        ├── val/
        └── test/
        """
        import logging
        
        # Always save JSON bounding boxes
        logging.info(f"Saving {len(self.bbox_controller.bboxes)} bounding boxes to JSON...")
        self.pcd_manager.save_labels_into_file(self.bbox_controller.bboxes)
        
        # Get export settings from EXPORT config section (set in startup dialog)
        data_format = config.get("EXPORT", "data_format", fallback="npy_folder")
        split = config.get("EXPORT", "default_split", fallback="train")
        
        # Export point cloud data
        if self.pcd_manager.pointcloud is not None:
            self._export_organized(data_format, split)
    ...
    def _export_organized(self, data_format: str, split: str) -> None:
        """Export data in organized train/val/test folder structure.
        
        Args:
            data_format: 'ply' or 'npy_folder'
        ...
        # Create organized output folder: segmentations/<split>/
        base_folder = config.getpath("FILE", "segmentation_folder")
        split_folder = base_folder / split
        split_folder.mkdir(parents=True, exist_ok=True)
        
        # Get point cloud labels (per-point)
        labels = pcd.labels
        if labels is None:
            labels = np.zeros(pcd.points.shape[0], dtype=np.int32)
        
        # Convert BBox objects to gt_boxes numpy array
        gt_boxes_np = None
        if bboxes:
            box_list = []
            for box in bboxes:
        ...
        scene_name = pcd.path.stem
        
        if data_format == "ply":
            # Export as PLY file
            output_path = split_folder / f"{scene_name}.ply"
            ...
            # Also export gt_boxes for detection support
            if gt_boxes_np is not None:
                box_path = split_folder / f"{scene_name}_gt_boxes.npy"
                np.save(box_path, gt_boxes_np)
            logging.info(f"PLY exported to {output_path}")
        else:
            # Export as NPY folder
            output_folder = split_folder / scene_name
            ...
            logging.info(f"NPY folder exported to {output_folder}")
        
        print(f"✅ Exported to: {split_folder / scene_name}* ({data_format} format, {split} split)")
    
    def _export_detection_npy(self) -> None:
        """Export detection data in NPY folder format (Object Detection mode).
        
        Creates:
        - coord.npy: Point coordinates
        - color.npy: Point colors
        - gt_boxes.npy: Bounding boxes [cx, cy, cz, l, w, h, rot_z, class_id]
        """
        import logging
        from ..io.segmentations.npy_folder import NPYFolderSegmentationHandler
        ...
        folder_path = label_folder / pcd.path.stem
        
        # Convert BBox objects to numpy array
        gt_boxes_np = None
        if bboxes:
            import numpy as np
            box_list = []
        ...
        # Create dummy labels (all zeros for detection-only mode)
        import numpy as np
        labels = np.zeros(pcd.points.shape[0], dtype=np.int8)
        ...
        logging.info(f"Detection NPY folder exported to {folder_path} with {len(bboxes)} boxes")
