...
class PointCloud(object):
    def __init__(
        ...
        self.labels = None
        # Enable segmentation labels for SEMANTIC_SEGMENTATION or UNIFIED mode
        if LabelConfig().type in (LabelingMode.SEMANTIC_SEGMENTATION, LabelingMode.UNIFIED):
    ...
    def save_segmentation_labels(self, extension=".ply", gt_boxes: Optional[List["BBox"]] = None) -> None:
        """
        Save segmentation labels.
        
        Args:
            extension: File extension/format to use. 
                - ".ply": Creates KPConvX-compatible PLY with embedded labels (recommended)
                - ".bin": Creates binary file with labels only (legacy format)
                - ".npy_folder" or "npy_folder": Creates LitePT-compatible NPY folder
            gt_boxes: List of BBox objects to export (for NPY format)
        """
        assert self.labels is not None
        self.validate_segmentation_label()
        
        # Handle NPY folder format for LitePT compatibility
        if extension in (".npy_folder", "npy_folder", "npy"):
            label_folder = config.getpath("FILE", "segmentation_folder")
            folder_path = label_folder / self.path.stem
            
            # Convert BBox objects to numpy array (M, 8)
            # [x, y, z, dx, dy, dz, heading, class]
            gt_boxes_np = None
            if gt_boxes:
                box_list = []
                for box in gt_boxes:
                    # Get class ID
                    try:
                        cls_id = LabelConfig().get_class(box.classname).id
                    except:
                        cls_id = -1
                        
                    # Get dimensions (dx, dy, dz)
                    l, w, h = box.get_dimensions()
                    
                    # Get rotation (convert degrees to radians)
                    rot_z = np.deg2rad(box.get_z_rotation())
                    
                    # Center
                    cx, cy, cz = box.center
                    
                    box_list.append([cx, cy, cz, l, w, h, rot_z, cls_id])
                
                if box_list:
                    gt_boxes_np = np.array(box_list, dtype=np.float32)

            from ..io.segmentations.npy_folder import NPYFolderSegmentationHandler
            NPYFolderSegmentationHandler.write_npy_folder(
            ...
            logging.info(f"Writing NPY folder (LitePT Unified format) to {folder_path} with {len(box_list) if gt_boxes else 0} boxes")
            return
        ...
        if extension == ".ply":
            # Use the PLY handler to write complete labeled point cloud
            from ..io.segmentations.ply import PLYSegmentationHandler
            PLYSegmentationHandler.write_labeled_ply(
                output_path=label_path,
                points=self.points,
                colors=self.colors,
                labels=self.labels
            )
            logging.info(f"Writing labeled PLY (KPConvX format) to {label_path}")
        else:
            # Legacy: use handler for the extension
    ...
    def from_file(
        ...
        labels = None
        # Load segmentation labels for SEMANTIC_SEGMENTATION or UNIFIED mode
        if LabelConfig().type in (LabelingMode.SEMANTIC_SEGMENTATION, LabelingMode.UNIFIED):
            seg_folder = config.getpath("FILE", "segmentation_folder")
            
            # Try PLY with labels  (KPConvX format)
            ply_label_path = seg_folder / f"{path.stem}.ply"
            # Try segment.npy in NPY folder (LitePT format)
            npy_label_path = seg_folder / path.stem / "segment.npy"
            
            if ply_label_path.exists():
                # Load from labeled PLY file
                logging.info(f"Loading segmentation labels from PLY: {ply_label_path}")
                try:
                    seg_handler = BaseSegmentationHandler.get_handler(".ply")()
                    labels = seg_handler.read_or_create_labels(
                        label_path=ply_label_path, num_points=points.shape[0]
                    )
                except Exception as e:
                    logging.warning(f"Failed to load labels from PLY: {e}")
                    labels = None
            
            if labels is None and npy_label_path.exists():
                # Load from NPY file
                logging.info(f"Loading segmentation labels from NPY: {npy_label_path}")
                try:
                    labels = np.load(npy_label_path).astype(np.int32)
                except Exception as e:
                    logging.warning(f"Failed to load labels from NPY: {e}")
                    labels = None
    ...
    def validate_segmentation_label(self) -> None:
        ...
        unique_class_ids.add(-1)  # Allow -1 (ignored/unlabeled) explicitly
